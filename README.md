...
```bash
pip install python-dotenv azure-ai-openai azure-storage-blob sqlalchemy pyodbc PyMuPDF docx2txt
```
...
AZURE_BLOB_KEY=your_azure_blob_storage_key
```

## How to Run
After installing all necessary libraries & database drivers and setting your ```.env``` file, run Jupyter Notebook in the project directory:
```jupyter notebook```